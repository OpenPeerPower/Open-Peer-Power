...
import openpeerpower.components.season.sensor as season
...
class TestSeason(unittest.TestCase):
    ...
    def test_setup_hemisphere_northern(self):
        ...
        self.opp.config.latitude = HEMISPHERE_NORTHERN["openpeerpower"]["latitude"]
        assert setup_component(self.opp, "sensor", HEMISPHERE_NORTHERN)
        assert (
            self.opp.config.as_dict()["latitude"]
            == HEMISPHERE_NORTHERN["openpeerpower"]["latitude"]
        )
        state = self.opp.states.get("sensor.season")
    ...
    def test_setup_hemisphere_southern(self):
        ...
        self.opp.config.latitude = HEMISPHERE_SOUTHERN["openpeerpower"]["latitude"]
        assert setup_component(self.opp, "sensor", HEMISPHERE_SOUTHERN)
        assert (
            self.opp.config.as_dict()["latitude"]
            == HEMISPHERE_SOUTHERN["openpeerpower"]["latitude"]
        )
        state = self.opp.states.get("sensor.season")
        assert state.attributes.get("friendly_name") == "Season"
    ...
    def test_setup_hemisphere_equator(self):
        ...
        self.opp.config.latitude = HEMISPHERE_EQUATOR["openpeerpower"]["latitude"]
        assert setup_component(self.opp, "sensor", HEMISPHERE_EQUATOR)
        assert (
            self.opp.config.as_dict()["latitude"]
            == HEMISPHERE_EQUATOR["openpeerpower"]["latitude"]
        )
        state = self.opp.states.get("sensor.season")
        assert state.attributes.get("friendly_name") == "Season"
    ...
    def test_setup_hemisphere_empty(self):
        ...
        self.opp.config.latitude = None
        assert setup_component(self.opp, "sensor", HEMISPHERE_EMPTY)
        assert self.opp.config.as_dict()["latitude"] is None
