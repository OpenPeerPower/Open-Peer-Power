...
from openpeerpower.components.zha.core.const import CONTROLLER, DOMAIN, ZHA_GW_RADIO
import openpeerpower.components.zha.core.registries
...
async def test_user_flow(opp):
    ...
    with asynctest.patch(
        "openpeerpower.components.zha.config_flow.check_zigpy_connection",
        return_value=False,
    ):
...
async def test_import_flow(opp):
...
async def test_import_flow_existing_config_entry(opp):
    """Test import from configuration.yaml ."""
    MockConfigEntry(domain=DOMAIN, data={"usb_path": "/dev/ttyUSB1"}).add_to_opp(opp)
    flow = config_flow.ZhaFlowHandler()
    flow.opp = opp
...
async def test_check_zigpy_connection():
    ...
    with mock.patch.dict(
        openpeerpower.components.zha.core.registries.RADIO_TYPES, new_radios, clear=True
    ):
