...
async def async_test_humidity(opp, cluster, entity_id):
    ...
    await send_attribute_report(opp, cluster, 0, 1000)
    assert_state(opp, entity_id, "10.0", "%")


async def async_test_temperature(opp, cluster, entity_id):
    """Test temperature sensor."""
    await send_attribute_report(opp, cluster, 0, 2900)
    assert_state(opp, entity_id, "29.0", "°C")


async def async_test_pressure(opp, cluster, entity_id):
    """Test pressure sensor."""
    await send_attribute_report(opp, cluster, 0, 1000)
    assert_state(opp, entity_id, "1000", "hPa")


async def async_test_illuminance(opp, cluster, entity_id):
    """Test illuminance sensor."""
    await send_attribute_report(opp, cluster, 0, 10)
    assert_state(opp, entity_id, "1.0", "lx")


async def async_test_metering(opp, cluster, entity_id):
    """Test metering sensor."""
    await send_attribute_report(opp, cluster, 1024, 12345)
...
async def async_test_electrical_measurement(opp, cluster, entity_id):
    ...
    with mock.patch(
        ...
        await send_attribute_report(opp, cluster, 1291, 100)
        assert_state(opp, entity_id, "100", "W")

        await send_attribute_report(opp, cluster, 1291, 99)
        assert_state(opp, entity_id, "99", "W")

        divisor_mock.return_value = 10
        await send_attribute_report(opp, cluster, 1291, 1000)
        assert_state(opp, entity_id, "100", "W")

        await send_attribute_report(opp, cluster, 1291, 99)
        assert_state(opp, entity_id, "9.9", "W")
...
async def send_attribute_report(opp, cluster, attrid, value):
