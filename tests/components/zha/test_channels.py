...
import openpeerpower.components.zha.core.device as zha_device
...
async def test_in_channel_config(
    cluster_id, bind_count, attrs, opp, zigpy_device_mock, zha_gateway
):
    ...
    zha_dev = zha_device.ZHADevice(opp, zigpy_dev, zha_gateway)
...
async def test_out_channel_config(
    cluster_id, bind_count, zha_gateway, opp, zigpy_device_mock
):
