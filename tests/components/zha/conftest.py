...
@pytest.fixture
def setup_zha(opp, config_entry, zigpy_app_controller, zigpy_radio):
    ...
    async def _setup():
        with mock.patch.dict(zha_regs.RADIO_TYPES, {"MockRadio": radio_details}):
            status = await async_setup_component(opp, zha_const.DOMAIN, zha_config)
...
@pytest.fixture
def zha_device_restored(opp, zigpy_app_controller, setup_zha):
