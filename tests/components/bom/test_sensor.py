...
from openpeerpower.components.bom.sensor import BOMCurrentData
...
class TestBOMWeatherSensor(unittest.TestCase):
    ...
    def test_setup(self, mock_get):
        ...
        with assert_setup_component(1, sensor.DOMAIN):
            assert setup_component(self.opp, sensor.DOMAIN, {"sensor": VALID_CONFIG})
        ...
        for entity_id in fake_entities:
            state = self.opp.states.get("sensor.{}".format(entity_id))
    ...
    def test_sensor_values(self, mock_get):
        ...
        weather = self.opp.states.get("sensor.bom_fake_weather").state
        assert "Fine" == weather

        pressure = self.opp.states.get("sensor.bom_fake_pressure_mb").state
        assert "1021.7" == pressure

        feels_like = self.opp.states.get("sensor.bom_fake_feels_like_c").state
