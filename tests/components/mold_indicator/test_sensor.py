...
class TestSensorMoldIndicator(unittest.TestCase):
    ...
    def test_sensor_changed(self):
        ...
        self.opp.block_till_done()
        assert self.opp.states.get("sensor.mold_indicator").state == "90"

        self.opp.states.set(
            "test.outdoortemp", "25", {ATTR_UNIT_OF_MEASUREMENT: TEMP_CELSIUS}
        )
        self.opp.block_till_done()
        assert self.opp.states.get("sensor.mold_indicator").state == "57"

        self.opp.states.set(
            "test.indoorhumidity", "20", {ATTR_UNIT_OF_MEASUREMENT: "%"}
        )
        self.opp.block_till_done()
        assert self.opp.states.get("sensor.mold_indicator").state == "23"
