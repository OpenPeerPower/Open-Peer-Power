...
from openpeerpower.generated import zeroconf as zc_gen
...
def mock_zeroconf():
    """Mock zeroconf."""
    with patch("openpeerpower.components.zeroconf.Zeroconf") as mock_zc:
...
async def test_setup(opp, mock_zeroconf):
    ...
    with patch.object(opp.config_entries, "flow") as mock_config_flow, patch.object(
...
async def test_homekit_match_partial(opp, mock_zeroconf):
    """Test configured options for a device are loaded via config entry."""
    with patch.dict(
        zc_gen.ZEROCONF, {zeroconf.HOMEKIT_TYPE: ["homekit_controller"]}, clear=True
    ), patch.object(opp.config_entries, "flow") as mock_config_flow, patch.object(
        zeroconf, "ServiceBrowser", side_effect=service_update_mock
    ) as mock_service_browser:
