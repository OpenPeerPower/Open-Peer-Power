...
class TestVacuumDemo(unittest.TestCase):
    ...
    def setUp(self):  # pylint: disable=invalid-name
        ...
        assert setup_component(self.opp, DOMAIN, {DOMAIN: {CONF_PLATFORM: "demo"}})
    ...
    def test_supported_features(self):
        ...
        state = self.opp.states.get(ENTITY_VACUUM_MOST)
        ...
        state = self.opp.states.get(ENTITY_VACUUM_BASIC)
        ...
        state = self.opp.states.get(ENTITY_VACUUM_MINIMAL)
        ...
        state = self.opp.states.get(ENTITY_VACUUM_NONE)
        ...
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
    ...
    def test_methods(self):
        ...
        self.opp.states.set(ENTITY_VACUUM_BASIC, STATE_ON)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_BASIC)

        self.opp.states.set(ENTITY_VACUUM_BASIC, STATE_OFF)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_BASIC)

        common.turn_on(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)

        common.turn_off(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)

        common.toggle(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)

        common.start_pause(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)
        ...
        common.stop(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)
        ...
        common.locate(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_COMPLETE)
        assert "I'm over here" in state.attributes.get(ATTR_STATUS)

        common.return_to_base(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_COMPLETE)
        ...
        common.clean_spot(self.opp, entity_id=ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_COMPLETE)
        assert "spot" in state.attributes.get(ATTR_STATUS)
        assert STATE_ON == state.state

        common.start(self.opp, ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert STATE_CLEANING == state.state

        common.pause(self.opp, ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert STATE_PAUSED == state.state

        common.stop(self.opp, ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        ...
        common.return_to_base(self.opp, ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert STATE_RETURNING == state.state

        common.set_fan_speed(self.opp, FAN_SPEEDS[-1], entity_id=ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert FAN_SPEEDS[-1] == state.attributes.get(ATTR_FAN_SPEED)

        common.clean_spot(self.opp, entity_id=ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert STATE_CLEANING == state.state
    ...
    def test_unsupported_methods(self):
        ...
        self.opp.states.set(ENTITY_VACUUM_NONE, STATE_ON)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_NONE)

        common.turn_off(self.opp, ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_NONE)

        common.stop(self.opp, ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_NONE)

        self.opp.states.set(ENTITY_VACUUM_NONE, STATE_OFF)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_NONE)

        common.turn_on(self.opp, ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_NONE)

        common.toggle(self.opp, ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_NONE)

        # Non supported methods:
        common.start_pause(self.opp, ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_NONE)

        common.locate(self.opp, ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_NONE)
        assert state.attributes.get(ATTR_STATUS) is None

        common.return_to_base(self.opp, ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_NONE)
        assert state.attributes.get(ATTR_STATUS) is None

        common.set_fan_speed(self.opp, FAN_SPEEDS[-1], entity_id=ENTITY_VACUUM_NONE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_NONE)
        assert FAN_SPEEDS[-1] != state.attributes.get(ATTR_FAN_SPEED)

        common.clean_spot(self.opp, entity_id=ENTITY_VACUUM_BASIC)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_BASIC)
        ...
        self.opp.states.set(ENTITY_VACUUM_COMPLETE, STATE_ON)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)

        common.pause(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        assert vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)

        self.opp.states.set(ENTITY_VACUUM_COMPLETE, STATE_OFF)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)

        common.start(self.opp, ENTITY_VACUUM_COMPLETE)
        self.opp.block_till_done()
        assert not vacuum.is_on(self.opp, ENTITY_VACUUM_COMPLETE)

        # StateVacuumDevice does not support on/off
        common.turn_on(self.opp, entity_id=ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert STATE_CLEANING != state.state

        common.turn_off(self.opp, entity_id=ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert STATE_RETURNING != state.state

        common.toggle(self.opp, entity_id=ENTITY_VACUUM_STATE)
        self.opp.block_till_done()
        state = self.opp.states.get(ENTITY_VACUUM_STATE)
        assert STATE_CLEANING != state.state
    ...
    def test_services(self):
        ...
        # Test send_command
        send_command_calls = mock_service(self.opp, DOMAIN, SERVICE_SEND_COMMAND)

        params = {"rotate": 150, "speed": 20}
        common.send_command(
            self.opp, "test_command", entity_id=ENTITY_VACUUM_BASIC, params=params
        )
        ...
        # Test set fan speed
        set_fan_speed_calls = mock_service(self.opp, DOMAIN, SERVICE_SET_FAN_SPEED)

        common.set_fan_speed(self.opp, FAN_SPEEDS[0], entity_id=ENTITY_VACUUM_COMPLETE)
    ...
    def test_set_fan_speed(self):
        ...
        old_state_basic = self.opp.states.get(ENTITY_VACUUM_BASIC)
        old_state_complete = self.opp.states.get(ENTITY_VACUUM_COMPLETE)
        old_state_state = self.opp.states.get(ENTITY_VACUUM_STATE)

        common.set_fan_speed(self.opp, FAN_SPEEDS[0], entity_id=group_vacuums)

        self.opp.block_till_done()
        new_state_basic = self.opp.states.get(ENTITY_VACUUM_BASIC)
        new_state_complete = self.opp.states.get(ENTITY_VACUUM_COMPLETE)
        new_state_state = self.opp.states.get(ENTITY_VACUUM_STATE)
    ...
    def test_send_command(self):
        ...
        common.send_command(
            self.opp, "test_command", params={"p1": 3}, entity_id=group_vacuums
        )
