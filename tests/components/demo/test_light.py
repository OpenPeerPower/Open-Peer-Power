...
async def test_state_attributes(opp):
    ...
    await common.async_turn_on(opp, ENTITY_LIGHT, xy_color=(0.4, 0.4), brightness=25)
    ...
    await common.async_turn_on(
        opp, ENTITY_LIGHT, rgb_color=(251, 253, 255), white_value=254
    )
    state = opp.states.get(ENTITY_LIGHT)
    ...
    await common.async_turn_on(opp, ENTITY_LIGHT, color_temp=400, effect="none")
    state = opp.states.get(ENTITY_LIGHT)
    ...
    await common.async_turn_on(opp, ENTITY_LIGHT, kelvin=3000, brightness_pct=50)
    state = opp.states.get(ENTITY_LIGHT)
