...
from openpeerpower.helpers import discovery, script
...
class TestNotifyDemo(unittest.TestCase):
    ...
    @patch("openpeerpower.components.demo.notify.get_service", autospec=True)
    def test_no_notify_service(self, mock_demo_get_service):
        ...
        with self.assertLogs(
            "openpeerpower.components.notify", level="ERROR"
        ) as log_handle:
            self._setup_notify()
        self.opp.block_till_done()
        assert mock_demo_get_service.called
        assert log_handle.output == [
            "ERROR:openpeerpower.components.notify:"
    ...
    def test_discover_notify(self, mock_demo_get_service):
        ...
        discovery.load_platform(
            self.opp, "notify", "demo", {"test_key": "test_val"}, {"notify": {}}
        )
        self.opp.block_till_done()
    ...
    def test_sending_none_message(self):
        ...
        with pytest.raises(vol.Invalid):
            common.send_message(self.opp, None)
        self.opp.block_till_done()
    ...
    def test_method_forwards_correct_data(self):
        ...
        self._setup_notify()
        common.send_message(self.opp, "my message", "my title", {"hello": "world"})
        self.opp.block_till_done()
    ...
    def test_calling_notify_from_script_loaded_from_yaml_without_title(self):
        ...
        script.call_from_config(self.opp, conf)
        self.opp.block_till_done()
        assert len(self.events) == 1
        assert {
            "message": "Test 123 4",
    ...
    def test_targets_are_services(self):
        ...
        self._setup_notify()
        assert self.opp.services.has_service("notify", "demo") is not None
        service = "demo_test_target_name"
        assert self.opp.services.has_service("notify", service) is not None

    def test_messages_to_targets_route(self):
        """Test message routing to specific target services."""
        self._setup_notify()
        self.opp.bus.listen_once("notify", self.record_calls)
