"""Test Z-Wave sensor."""
from openpeerpower.components.zwave import const, sensor
import openpeerpower.const
...
def test_get_device_detects_battery_sensor(mock_openzwave):
    ...
    assert device.device_class == openpeerpower.const.DEVICE_CLASS_BATTERY
...
def test_multilevelsensor_value_changed_temp_celsius(mock_openzwave):
    ...
    assert device.unit_of_measurement == openpeerpower.const.TEMP_CELSIUS
    value.data = 37.95555
    value_changed(value)
