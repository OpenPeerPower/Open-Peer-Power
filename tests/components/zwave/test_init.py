...
from openpeerpower.components import zwave
from openpeerpower.components.zwave import (
    CONF_DEVICE_CONFIG_GLOB,
    CONFIG_SCHEMA,
    DATA_NETWORK,
    const,
)
from openpeerpower.components.zwave.binary_sensor import get_device
...
async def test_invalid_device_config(opp, mock_openzwave):
...
async def test_auto_heal_midnight(opp, mock_openzwave):
...
async def test_setup_platform(opp, mock_openzwave):
    ...
    result = await zwave.async_setup_platform(
        opp, None, async_add_entities, {const.DISCOVERY_DEVICE: 123}
    )
    assert not result
    assert not async_add_entities.called

    result = await zwave.async_setup_platform(
        opp, None, async_add_entities, {const.DISCOVERY_DEVICE: 456}
    )
    assert result
...
async def test_zwave_ready_wait(opp, mock_openzwave):
    ...
    with patch("openpeerpower.components.zwave.dt_util.utcnow", new=utcnow):
...
async def test_node_discovery(opp, mock_openzwave):
    ...
    opp.async_add_job(mock_receivers[0], node)
    await opp.async_block_till_done()
...
async def test_value_entities(opp, mock_openzwave):
    ...
    opp.async_add_job(mock_receivers[MockNetwork.SIGNAL_ALL_NODES_QUERIED])
    node = MockNode(node_id=11, generic=const.GENERIC_TYPE_SENSOR_BINARY)
    ...
    opp.async_add_job(mock_receivers[MockNetwork.SIGNAL_NODE_ADDED], node)
    opp.async_add_job(mock_receivers[MockNetwork.SIGNAL_VALUE_ADDED], node, value)
    opp.async_add_job(mock_receivers[MockNetwork.SIGNAL_VALUE_ADDED], node, value2)
    await opp.async_block_till_done()
...
async def test_value_discovery_existing_entity(opp, mock_openzwave):
    ...
    opp.async_add_job(mock_receivers[0], node, thermostat_mode)
    await opp.async_block_till_done()

    def mock_update(self):
        self.opp.add_job(self.async_update_op_state)

    with patch.object(
        zwave.node_entity.ZWaveBaseEntity, "maybe_schedule_update", new=mock_update
    ):
        opp.async_add_job(mock_receivers[0], node, setpoint_heating)
        await opp.async_block_till_done()
    ...
    with patch.object(
        ...
        opp.async_add_job(mock_receivers[0], node, temperature)
        await opp.async_block_till_done()
...
async def test_power_schemes(opp, mock_openzwave):
    ...
    opp.async_add_job(mock_receivers[0], node, switch)
    ...
    with patch.object(
        ...
        opp.async_add_job(mock_receivers[0], node, power)
        await opp.async_block_till_done()

    assert (
        opp.states.get("switch.mock_node_mock_value").attributes["power_consumption"]
        == 23.5
    )
...
async def test_network_ready(opp, mock_openzwave):
    ...
    opp.bus.async_listen(const.EVENT_NETWORK_COMPLETE, listener)

    opp.async_add_job(mock_receivers[0])
    await opp.async_block_till_done()
...
async def test_network_complete(opp, mock_openzwave):
    ...
    opp.bus.async_listen(const.EVENT_NETWORK_READY, listener)
...
async def test_network_complete_some_dead(opp, mock_openzwave):
    ...
    opp.bus.async_listen(const.EVENT_NETWORK_COMPLETE_SOME_DEAD, listener)
...
class TestZWaveDeviceEntityValues(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.opp.start()
        self.registry = mock_registry(self.opp)

        setup_component(self.opp, "zwave", {"zwave": {}})
...
class TestZWaveServices(unittest.TestCase):
    ...
    def setUp(self):
        ...
        setup_component(self.opp, "zwave", {"zwave": {}})
        self.opp.block_till_done()
        self.zwave_network = self.opp.data[DATA_NETWORK]
        self.zwave_network.state = MockNetwork.STATE_READY
        self.opp.bus.fire(EVENT_OPENPEERPOWER_START)
        self.opp.block_till_done()

    def tearDown(self):  # pylint: disable=invalid-name
        """Stop everything that was started."""
        self.opp.services.call("zwave", "stop_network", {})
        self.opp.block_till_done()
        self.opp.stop()

    def test_add_node(self):
        """Test zwave add_node service."""
        self.opp.services.call("zwave", "add_node", {})
        self.opp.block_till_done()
    ...
    def test_add_node_secure(self):
        ...
        self.opp.services.call("zwave", "add_node_secure", {})
        self.opp.block_till_done()
    ...
    def test_remove_node(self):
        ...
        self.opp.services.call("zwave", "remove_node", {})
        self.opp.block_till_done()
    ...
    def test_cancel_command(self):
        ...
        self.opp.services.call("zwave", "cancel_command", {})
        self.opp.block_till_done()
    ...
    def test_heal_network(self):
        ...
        self.opp.services.call("zwave", "heal_network", {})
        self.opp.block_till_done()
    ...
    def test_soft_reset(self):
        ...
        self.opp.services.call("zwave", "soft_reset", {})
        self.opp.block_till_done()
    ...
    def test_test_network(self):
        ...
        self.opp.services.call("zwave", "test_network", {})
        self.opp.block_till_done()
    ...
    def test_stop_network(self):
        ...
        with patch.object(self.opp.bus, "fire") as mock_fire:
            self.opp.services.call("zwave", "stop_network", {})
            self.opp.block_till_done()
    ...
    def test_remove_failed_node(self):
        ...
        self.opp.services.call("zwave", "remove_failed_node", {const.ATTR_NODE_ID: 12})
        self.opp.block_till_done()
    ...
    def test_print_node(self):
        ...
        with self.assertLogs(level="DEBUG") as mock_logger:
            self.opp.services.call("zwave", "print_node", {const.ATTR_NODE_ID: 14})
            self.opp.block_till_done()
    ...
    def test_reset_node_meters(self):
        ...
        self.opp.services.call("zwave", "reset_node_meters", {const.ATTR_NODE_ID: 14})
        self.opp.block_till_done()
    ...
    def test_refresh_entity(self):
        ...
        self.opp.add_job(device.async_added_to_opp())
        self.opp.block_till_done()
    ...
    def test_refresh_node(self):
        ...
        node = MockNode(node_id=14)
        self.zwave_network.nodes = {14: node}
        self.opp.services.call("zwave", "refresh_node", {const.ATTR_NODE_ID: 14})
        self.opp.block_till_done()
    ...
    def test_heal_node(self):
        ...
        self.opp.services.call("zwave", "heal_node", {const.ATTR_NODE_ID: 19})
        self.opp.block_till_done()
    ...
    def test_test_node(self):
        ...
        node = MockNode(node_id=19)
        self.zwave_network.nodes = {19: node}
        self.opp.services.call("zwave", "test_node", {const.ATTR_NODE_ID: 19})
        self.opp.block_till_done()
