...
from openpeerpower.components.cloud import ALEXA_SCHEMA, alexa_config
...
async def test_alexa_config_expose_entity_prefs(opp, cloud_prefs):
    ...
    conf = alexa_config.AlexaConfig(opp, ALEXA_SCHEMA({}), cloud_prefs, None)
...
def patch_sync_helper():
    ...
    with patch("openpeerpower.components.cloud.alexa_config.SYNC_DELAY", 0), patch(
        "openpeerpower.components.cloud.alexa_config.AlexaConfig._sync_helper",
...
async def test_alexa_entity_registry_sync(opp, mock_cloud_login, cloud_prefs):
    ...
    alexa_config.AlexaConfig(opp, ALEXA_SCHEMA({}), cloud_prefs, opp.data["cloud"])
...
async def test_alexa_update_report_state(opp, cloud_prefs):
    ...
    with patch(
        ...
        "openpeerpower.components.cloud.alexa_config."
