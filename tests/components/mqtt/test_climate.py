...
async def test_custom_availability_payload(opp, mqtt_mock):
    ...
    state = opp.states.get("climate.test")
...
async def test_set_target_temperature_low_high_with_templates(opp, mqtt_mock, caplog):
...
async def test_discovery_update_attr(opp, mqtt_mock, caplog):
    ...
    async_fire_mqtt_message(opp, "openpeerpower/climate/bla/config", data1)
    await opp.async_block_till_done()
    async_fire_mqtt_message(opp, "attr-topic1", '{ "val": "100" }')
    state = opp.states.get("climate.beer")
    assert state.attributes.get("val") == "100"

    # Change json_attributes_topic
    async_fire_mqtt_message(opp, "openpeerpower/climate/bla/config", data2)
    await opp.async_block_till_done()
...
async def test_unique_id(opp):
    ...
    assert len(opp.states.async_entity_ids(CLIMATE_DOMAIN)) == 1
...
async def test_discovery_removal_climate(opp, mqtt_mock, caplog):
    ...
    async_fire_mqtt_message(opp, "openpeerpower/climate/bla/config", "")
    await opp.async_block_till_done()
    state = opp.states.get("climate.beer")
...
async def test_discovery_update_climate(opp, mqtt_mock, caplog):
    ...
    state = opp.states.get("climate.milk")
    assert state is None
