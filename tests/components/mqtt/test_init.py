...
def mock_MQTT():
    ...
    with mock.patch("openpeerpower.components.mqtt.MQTT") as mock_MQTT:
        mock_MQTT.return_value.async_connect.return_value = mock_coro(True)
        yield mock_MQTT


async def async_mock_mqtt_client(opp, config=None):
    ...
    with mock.patch("paho.mqtt.client.Client") as mock_client:
        ...
        result = await async_setup_component(opp, mqtt.DOMAIN, {mqtt.DOMAIN: config})
        assert result
...
class TestMQTTComponent(unittest.TestCase):
    ...
    def aiohttp_client_stops_on_open_peer_power_start(self):
        """Test if client stops on HA stop."""
        self.opp.bus.fire(EVENT_OPENPEERPOWER_STOP)
        self.opp.block_till_done()
        assert self.opp.data["mqtt"].async_disconnect.called

    def test_publish_calls_service(self):
        """Test the publishing of call to services."""
        self.opp.bus.listen_once(EVENT_CALL_SERVICE, self.record_calls)

        mqtt.publish(self.opp, "test-topic", "test-payload")
    ...
    def test_service_call_without_topic_does_not_publish(self):
        ...
        self.opp.block_till_done()
        assert not self.opp.data["mqtt"].async_publish.called
    ...
    def test_service_call_with_template_payload_renders_template(self):
        ...
        """
        mqtt.publish_template(self.opp, "test/topic", "{{ 1+1 }}")
        self.opp.block_till_done()
        assert self.opp.data["mqtt"].async_publish.called
        assert self.opp.data["mqtt"].async_publish.call_args[0][1] == "2"
    ...
    def test_service_call_with_ascii_qos_retain_flags(self):
        ...
        assert self.opp.data["mqtt"].async_publish.called
        assert self.opp.data["mqtt"].async_publish.call_args[0][2] == 2
        assert not self.opp.data["mqtt"].async_publish.call_args[0][3]
...
class TestMQTTCallbacks(unittest.TestCase):
    ...
    def aiohttp_client_starts_on_open_peer_power_mqtt_setup(self):
        """Test if client is connected after mqtt init on bootstrap."""
        assert self.opp.data["mqtt"]._mqttc.connect.call_count == 1
    ...
    def test_all_subscriptions_run_when_decode_fails(self):
        ...
        mqtt.subscribe(self.opp, "test-topic", self.record_calls, encoding="ascii")
        mqtt.subscribe(self.opp, "test-topic", self.record_calls)
    ...
    def test_subscribe_topic_level_wildcard_root_topic_no_subtree_match(self):
        ...
        fire_mqtt_message(self.opp, "test-topic-123", "test-payload")
    ...
    def test_subscribe_topic_level_wildcard_and_wildcard_root_topic(self):
        ...
        fire_mqtt_message(self.opp, "hi/test-topic", "test-payload")
    ...
    def test_subscribe_topic_level_wildcard_and_wildcard_level_no_match(self):
        ...
        fire_mqtt_message(self.opp, "hi/here-iam/test-topic", "test-payload")
    ...
    def test_subscribe_topic_sys_root_and_wildcard_subtree_topic(self):
        ...
        mqtt.subscribe(self.opp, "$test-topic/subtree/#", self.record_calls)

        fire_mqtt_message(self.opp, "$test-topic/subtree/some-topic", "test-payload")
    ...
    def test_mqtt_disconnect_tries_no_reconnect_on_stop(self):
        ...
        assert not self.opp.data["mqtt"]._mqttc.reconnect.called

    @mock.patch("openpeerpower.components.mqtt.time.sleep")
    def test_mqtt_disconnect_tries_reconnect(self, mock_sleep):
        """Test the re-connect tries."""
        self.opp.data["mqtt"].subscriptions = [
            mqtt.Subscription("test/progress", None, 0),
            mqtt.Subscription("test/progress", None, 1),
            mqtt.Subscription("test/topic", None, 2),
        ]
        self.opp.data["mqtt"]._mqttc.reconnect.side_effect = [1, 1, 1, 0]
        self.opp.data["mqtt"]._mqtt_on_disconnect(None, None, 1)
        assert self.opp.data["mqtt"]._mqttc.reconnect.called
        assert len(self.opp.data["mqtt"]._mqttc.reconnect.mock_calls) == 4
...
async def test_setup_embedded_starts_with_no_config(opp):
    """Test setting up embedded server with no config."""
    client_config = ("localhost", 1883, "user", "pass", None, "3.1.1")

    with mock.patch(
        "openpeerpower.components.mqtt.server.async_start",
        return_value=mock_coro(return_value=(True, client_config)),
    ) as _start:
        await async_mock_mqtt_client(opp, {})
        assert _start.call_count == 1


async def test_setup_embedded_with_embedded(opp):
    """Test setting up embedded server with no config."""
    client_config = ("localhost", 1883, "user", "pass", None, "3.1.1")
    ...
    with mock.patch(
        ...
        await async_mock_mqtt_client(opp, {"embedded": None})
        assert _start.call_count == 1


async def test_setup_fails_if_no_connect_broker(opp):
    """Test for setup failure if connection to broker is missing."""
    entry = MockConfigEntry(domain=mqtt.DOMAIN, data={mqtt.CONF_BROKER: "test-broker"})

    with mock.patch("paho.mqtt.client.Client") as mock_client:
        mock_client().connect = lambda *args: 1
        assert not await mqtt.async_setup_entry(opp, entry)


async def test_setup_raises_ConfigEntryNotReady_if_no_connect_broker(opp):
    """Test for setup failure if connection to broker is missing."""
    entry = MockConfigEntry(domain=mqtt.DOMAIN, data={mqtt.CONF_BROKER: "test-broker"})
    ...
    with mock.patch("paho.mqtt.client.Client") as mock_client:
        ...
        with pytest.raises(ConfigEntryNotReady):
            await mqtt.async_setup_entry(opp, entry)


async def test_setup_uses_certificate_on_certificate_set_to_auto(opp, mock_MQTT):
...
async def test_setup_does_not_use_certificate_on_mqtts_port(opp, mock_MQTT):
...
async def test_setup_without_tls_config_uses_tlsv1_under_python36(opp, mock_MQTT):
...
async def test_setup_with_tls_config_uses_tls_version1_2(opp, mock_MQTT):
...
async def test_setup_with_tls_config_of_v1_under_python36_only_uses_v1(opp, mock_MQTT):
...
async def test_birth_message(opp):
    ...
    opp.data["mqtt"]._mqtt_on_connect(None, None, 0, 0)
    await opp.async_block_till_done()
    assert calls[-1] == ("birth", "birth", 0, False)


async def test_mqtt_subscribes_topics_on_connect(opp):
    """Test subscription to topic on connect."""
    mqtt_client = await async_mock_mqtt_client(opp)

    opp.data["mqtt"].subscriptions = [
...
async def test_message_callback_exception_gets_logged(opp, caplog):
...
async def test_mqtt_ws_subscription(opp, opp_ws_client):
...
async def test_dump_service(opp):
    ...
    with mock.patch("openpeerpower.components.mqtt.open", mock_open):
