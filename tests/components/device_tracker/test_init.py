...
async def test_device_hidden(opp, mock_device_tracker_conf):
    """Test hidden devices."""
    devices = mock_device_tracker_conf
    dev_id = "test_entity"
    entity_id = const.ENTITY_ID_FORMAT.format(dev_id)
    device = legacy.Device(
        opp, timedelta(seconds=180), True, dev_id, None, hide_if_away=True
    )
    devices.append(device)
    ...
    assert opp.states.get(entity_id).attributes.get(ATTR_HIDDEN)
...
async def test_default_hide_if_away_is_used(mock_device_tracker_conf, opp):
    ...
    tracker = legacy.DeviceTracker(
        opp, timedelta(seconds=60), False, {device_tracker.CONF_AWAY_HIDE: True}, []
    )
