...
DOCUMENTATION_URL_HOST = "www.open-peer-power.io"
DOCUMENTATION_URL_PATH_PREFIX = "/integrations/"
DOCUMENTATION_URL_EXCEPTIONS = ["https://www.open-peer-power.io/oppio"]
...
def documentation_url(value: str) -> str:
    ...
    if not parsed_url.netloc == DOCUMENTATION_URL_HOST:
        raise vol.Invalid("Documentation url not hosted at www.open-peer-power.io")
    if not parsed_url.path.startswith(DOCUMENTATION_URL_PATH_PREFIX):
        raise vol.Invalid(
            "Documentation url does not begin with www.open-peer-power.io/integrations"
        )
