...
CONSTRAINT_PATH = os.path.join(
    os.path.dirname(__file__), "../openpeerpower/package_constraints.txt"
)
CONSTRAINT_BASE = """
pycryptodome>=3.6.6
...
def has_tests(module: str):
    ...
    Module format: openpeerpower.components.hue
    Test if exists: tests/components/hue
    """
    path = Path(module.replace(".", "/").replace("openpeerpower", "tests"))
    if not path.exists():
        return False
...
def gather_recursive_requirements(domain, seen=None):
    ...
    seen.add(domain)
    integration = Integration(Path(f"openpeerpower/components/{domain}"))
...
def gather_requirements_from_manifests(errors, reqs):
    ...
    integrations = Integration.load_dir(Path("openpeerpower/components"))
    for domain in sorted(integrations):
        ...
        process_requirements(
            ...
            f"openpeerpower.components.{domain}",
            reqs,
        )
...
def gather_requirements_from_modules(errors, reqs):
    ...
    for package in sorted(
        explore_module("openpeerpower.scripts", True)
        + explore_module("openpeerpower.auth", True)
    ):
        try:
            module = importlib.import_module(package)
...
def requirements_all_output(reqs):
    ...
    output = []
    output.append("# Open Peer Power core")
    output.append("\n")
...
def requirements_test_output(reqs):
    ...
    output = []
    output.append("# Open Peer Power tests, full dependency set\n")
    output.append(
    ...
    filtered = {
        ...
        if any(
            ...
            not mdl.startswith("openpeerpower.components.") or
...
def main(validate):
    ...
    files = (
        ...
        ("openpeerpower/package_constraints.txt", constraints),
    )
