...
During startup, Open Peer Power will setup the entries during the normal setup
of a component. It will first call the normal setup and then call the method
`async_setup_entry(opp, entry)` for each entry. The same method is called when
Open Peer Power is running while a config entry is created.  If the version of
the config entry does not match that of the flow handler, setup will
call the method `async_migrate_entry(opp, entry)` with the expectation that
...
from user input, discovery or other sources (like oppio).
...
Before instantiating the handler, Open Peer Power will make sure to load all
...
configuration flow is finished.
    ...
    return self.async_abort(
        reason='This device is not supported by Open Peer Power.'
    )
...
from openpeerpower.exceptions import OpenPeerPowerError, ConfigEntryNotReady
from openpeerpower.setup import async_setup_component, async_process_deps_reqs
...
class ConfigError(OpenPeerPowerError):
...
class ConfigEntry:
        ...
        try:
            ...
            async def setup_again(now):
                ...
                await self.async_setup(
                    opp, integration=integration, tries=tries)

            self._async_cancel_retry_setup = \
                opp.helpers.event.async_call_later(wait_time, setup_again)
            return
    ...
    async def async_unload(self, opp, *, integration=None) -> bool:
        """Unload an entry.
        ...
        try:
            result = await component.async_unload_entry(opp, self)
    ...
    async def async_migrate(self, opp: OpenPeerPower) -> bool:
        ...
        component = getattr(opp.components, self.domain)
        ...
        try:
            result = await component.async_migrate_entry(opp, self)
            if not isinstance(result, bool):
            ...
            if result:
                ...
                opp.config_entries._async_schedule_save()  # type: ignore
            return result
        except Exception:  # pylint: disable=broad-except
    ...
    def add_update_listener(self, listener: Callable) -> Callable:
        ...
        Listener: Callback function(opp, entry)
...
class ConfigEntries:
    ...
    def __init__(self, opp: OpenPeerPower, opp_config: dict) -> None:
        ...
        self.flow = data_entry_flow.FlowManager(
            opp, self._async_create_flow, self._async_finish_flow)
    ...
    async def async_remove(self, entry_id):
        ...
        await entry.async_remove(self.opp)
    ...
    async def async_initialize(self) -> None:
        ...
        config = await self.opp.helpers.storage.async_migrator(
            self.opp.config.path(PATH_CONFIG), self._store,
    ...
    async def async_setup(self, entry_id: str) -> bool:
        ...
        if entry.domain in self.opp.config.components:
            await entry.async_setup(self.opp)
        else:
    ...
    async def async_unload(self, entry_id: str) -> bool:
        ...
        return await entry.async_unload(self.opp)
    ...
    async def async_forward_entry_setup(self, entry, domain):
        ...
        if domain not in self.opp.config.components:
            result = await async_setup_component(
                self.opp, domain, self._opp_config)
    ...
    async def _async_finish_flow(self, flow, result):
        ...
        if not any(ent['context']['source'] in DISCOVERY_SOURCES for ent
                   in self.opp.config_entries.flow.async_progress()
    ...
    async def _async_create_flow(self, handler_key, *, context, data):
        ...
        await async_process_deps_reqs(
            self.opp, self._opp_config, integration)
        ...
        if source in DISCOVERY_SOURCES:
            self.opp.bus.async_fire(EVENT_FLOW_DISCOVERED)
...
class ConfigFlow(data_entry_flow.FlowHandler):
    ...
    def _async_current_entries(self):
        ...
        return self.opp.config_entries.async_entries(self.handler)

    @callback
    def _async_in_progress(self):
        """Return other in progress flows for current domain."""
        return [flw for flw in self.opp.config_entries.flow.async_progress()
...
class OptionsFlowManager:
    ...
    async def _async_create_flow(self, entry_id, *, context, data):
        ...
        """
        entry = self.opp.config_entries.async_get_entry(entry_id)
        if entry is None:
            return
    ...
    async def _async_finish_flow(self, flow, result):
        ...
        """
        entry = self.opp.config_entries.async_get_entry(flow.handler)
        if entry is None:
            return
