...
from openpeerpower.setup import async_process_deps_reqs, async_setup_component
...
class ConfigEntries:
    ...
    async def async_initialize(self) -> None:
        ...
        config = await self.opp.helpers.storage.async_migrator(
            self.opp.config.path(PATH_CONFIG),
            self._store,
    ...
    async def _async_finish_flow(
        ...
        if not any(
            ...
            for ent in self.opp.config_entries.flow.async_progress()
...
class ConfigFlow(data_entry_flow.FlowHandler):
    ...
    def _async_current_ids(self, include_ignore: bool = True) -> Set[Optional[str]]:
        ...
        return set(
            entry.unique_id
            for entry in self.opp.config_entries.async_entries(self.handler)
    ...
    def _async_in_progress(self) -> List[Dict]:
        ...
        return [
            flw
            for flw in self.opp.config_entries.flow.async_progress()
...
class OptionsFlowManager:
    ...
    async def _async_finish_flow(
        ...
        if entry is None:
            return None
        self.opp.config_entries.async_update_entry(entry, options=result["data"])
...
class EntityRegistryDisabledHandler:
    ...
    async def _handle_reload(self, _now: Any) -> None:
        ...
        await asyncio.gather(
            *[self.opp.config_entries.async_reload(entry_id) for entry_id in to_reload]
        )
