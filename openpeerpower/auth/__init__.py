...
async def auth_manager_from_config(
    ...
    if provider_configs:
        providers = await asyncio.gather(
            *(
                auth_provider_from_config(opp, store, config)
    ...
    if module_configs:
        modules = await asyncio.gather(
            *(auth_mfa_module_from_config(opp, config) for config in module_configs)
        )
    else:
        modules = []
...
class AuthManager:
    ...
    async def async_create_system_user(
        ...
        self.opp.bus.async_fire(EVENT_USER_ADDED, {"user_id": user.id})
    ...
    async def async_remove_user(self, user: models.User) -> None:
        ...
        self.opp.bus.async_fire(EVENT_USER_REMOVED, {"user_id": user.id})
