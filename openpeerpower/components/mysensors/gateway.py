...
from openpeerpower.const import CONF_OPTIMISTIC, EVENT_OPENPEERPOWER_STOP
...
async def setup_gateways(opp, config):
    ...
    for index, gateway_conf in enumerate(conf[CONF_GATEWAYS]):
        ...
        ready_gateway = await _get_gateway(opp, config, gateway_conf, persistence_file)
...
async def finish_setup(opp, opp_config, gateways):
    ...
    for gateway in gateways.values():
        discover_tasks.append(_discover_persistent_devices(opp, opp_config, gateway))
...
async def _discover_persistent_devices(opp, opp_config, gateway):
    ...
    for platform, dev_ids in new_devices.items():
        tasks.append(discover_mysensors_platform(opp, opp_config, platform, dev_ids))
    if tasks:
