...
class AlexaConfig(alexa_config.AbstractConfig):
    ...
    def __init__(self, opp, config, prefs, cloud):
    ...
    async def _async_prefs_updated(self, prefs):
        ...
        self._alexa_sync_unsub = async_call_later(
            self.opp, SYNC_DELAY, self._sync_prefs
        )
    ...
    async def async_sync_entities(self):
        ...
        for entity in alexa_entities.async_get_entities(self.opp, self):
        ...
        if to_update:
            tasks.append(
                alexa_state_report.async_send_add_or_update_message(
                    self.opp, self, to_update
                )
            )

        if to_remove:
            tasks.append(
                alexa_state_report.async_send_delete_message(self.opp, self, to_remove)
            )
