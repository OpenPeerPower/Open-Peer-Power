...
from openpeerpower.components.climate import PLATFORM_SCHEMA, ClimateDevice
...
from openpeerpower.core import DOMAIN as HA_DOMAIN, callback
...
class GenericThermostat(ClimateDevice, RestoreEntity):
    ...
    async def async_added_to_opp(self):
        ...
        # Add listener
        async_track_state_change(
            self.opp, self.sensor_entity_id, self._async_sensor_changed
        )
        async_track_state_change(
            self.opp, self.heater_entity_id, self._async_switch_changed
        )

        if self._keep_alive:
            async_track_time_interval(
                self.opp, self._async_control_heating, self._keep_alive
            )
        ...
        self.opp.bus.async_listen_once(EVENT_OPENPEERPOWER_START, _async_startup)
    ...
    def _is_device_active(self):
        ...
        return self.opp.states.is_state(self.heater_entity_id, STATE_ON)
    ...
    async def _async_heater_turn_on(self):
        ...
        await self.opp.services.async_call(HA_DOMAIN, SERVICE_TURN_ON, data)

    async def _async_heater_turn_off(self):
        """Turn heater toggleable device off."""
        data = {ATTR_ENTITY_ID: self.heater_entity_id}
        await self.opp.services.async_call(HA_DOMAIN, SERVICE_TURN_OFF, data)
