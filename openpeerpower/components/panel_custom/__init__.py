...
async def async_setup(opp, config):
    ...
    for panel in config[DOMAIN]:
        ...
        if panel_path is None:
            panel_path = opp.config.path(PANEL_DIR, f"{name}.html")
        ...
        elif not await opp.async_add_job(os.path.isfile, panel_path):
            _LOGGER.error("Unable to find webcomponent for %s: %s", name, panel_path)
            continue

        else:
            url = LEGACY_URL.format(name)
            opp.http.register_static_path(url, panel_path)
            kwargs["html_url"] = url
