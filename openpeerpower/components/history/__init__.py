...
from openpeerpower.components import recorder, script
from openpeerpower.components.http import HomeAssistantView
from openpeerpower.const import ATTR_HIDDEN
from openpeerpower.components.recorder.util import session_scope, execute
...
def get_significant_states(opp, start_time, end_time=None, entity_ids=None,
    ...
    return states_to_json(
        opp, states, start_time, entity_ids, filters,
        include_start_time_state)


def state_changes_during_period(opp, start_time, end_time=None,
                                entity_id=None):
    """Return states changes during UTC period start_time - end_time."""
    from openpeerpower.components.recorder.models import States
...
def get_last_state_changes(opp, number_of_states, entity_id):
    ...
    return states_to_json(opp, reversed(states),
                          start_time,
                          entity_ids,
                          include_start_time_state=False)


def get_states(opp, utc_point_in_time, entity_ids=None, run=None,
               filters=None):
...
def states_to_json(
    ...
    if include_start_time_state:
        for state in get_states(opp, start_time, entity_ids, filters=filters):
...
def get_state(opp, utc_point_in_time, entity_id, run=None):
    ...
    states = list(get_states(opp, utc_point_in_time, (entity_id,), run))
...
class HistoryPeriodView(HomeAssistantView):
    ...
    async def get(self, request, datetime=None):
        ...
        result = await opp.async_add_job(
            get_significant_states, opp, start_time, end_time,
