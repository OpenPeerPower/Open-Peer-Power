...
class SpeechManager:
    ...
    async def async_init_cache(self, use_cache, cache_dir, time_memory, base_url):
        ...
        try:
            ...
            raise OpenPeerPowerError(f"Can't init cache dir {err}")
        ...
        try:
            ...
            raise OpenPeerPowerError(f"Can't read cache dir {err}")
    ...
    async def async_get_tts_audio(self, engine, key, message, cache, language, options):
        ...
        if cache:
            self.opp.async_create_task(self.async_save_tts_audio(key, filename, data))
    ...
    async def async_file_to_mem(self, key):
        ...
        if not filename:
            raise OpenPeerPowerError(f"Key {key} not in file cache!")
        ...
        try:
            ...
            raise OpenPeerPowerError(f"Can't read {voice_file}")
    ...
    async def async_read_tts(self, filename):
        ...
        if key not in self.mem_cache:
            if key not in self.file_cache:
                raise OpenPeerPowerError(f"{key} not in cache!")
