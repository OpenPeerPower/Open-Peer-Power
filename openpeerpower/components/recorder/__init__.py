...
async def async_setup(opp: OpenPeerPower, config: ConfigType) -> bool:
    ...
    instance = opp.data[DATA_INSTANCE] = Recorder(
        opp=opp, keep_days=keep_days, purge_interval=purge_interval,
...
class Recorder(threading.Thread):
    ...
    def async_initialize(self):
        """Initialize the recorder."""
        self.opp.bus.async_listen(MATCH_ALL, self.event_listener)
    ...
    def run(self):
        ...
        if self.keep_days and self.purge_interval:
            ...
            def async_purge(now):
                ...
                self.opp.helpers.event.async_track_point_in_time(
            ...
            self.opp.helpers.event.track_point_in_time(async_purge, run)
