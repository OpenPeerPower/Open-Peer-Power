...
class ZWaveClimateBase(ZWaveDeviceEntity, ClimateDevice):
    ...
    def __init__(self, values, temp_unit):
        ...
        self._hvac_mapping = None  # {op_mode:zwave_mode}
        self._hvac_mode = None  # op_mode
        self._aux_heat = None
        self._default_hvac_mode = None  # op_mode
        self._preset_mapping = None  # {op_mode:zwave_mode}
        self._preset_list = None  # [zwave_mode]
        self._preset_mode = None  # op_mode if exists, else zwave_mode
        ...
        if self._mode():
            ...
            if mode_list:
                for mode in mode_list:
                    op_mode = HVAC_STATE_MAPPINGS.get(str(mode).lower())
                    op_preset = PRESET_MAPPINGS.get(str(mode).lower())
                    if mode == AUX_HEAT_ZWAVE_MODE:
                        # Aux Heat should not be included in any mapping
                        self._aux_heat = True
                    elif op_mode and op_mode not in self._hvac_mapping:
                        self._hvac_mapping[op_mode] = mode
                        self._hvac_list.append(op_mode)
                    elif op_preset and op_preset not in self._preset_mapping:
                        self._preset_mapping[op_preset] = mode
                        self._preset_list.append(op_preset)
                    else:
                        # If nothing matches
