...
from openpeerpower.components.group import ENTITY_ID_FORMAT as GROUP_ENTITY_ID_FORMAT
...
from openpeerpower.exceptions import Unauthorized, UnknownUser
...
async def async_setup(opp, config):
    ...
    async def async_handle_light_on_service(service):
        ...
        if poll_lights:
            await asyncio.wait(
                [light.async_update_op_state(True) for light in poll_lights]
            )
