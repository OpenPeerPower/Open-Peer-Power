...
from openpeerpower.helpers import event, service
...
async def async_setup(opp, config):
    ...
    tasks = [alert.async_update_op_state() for alert in entities]
    if tasks:
...
class Alert(ToggleEntity):
    ...
    async def _send_notification_message(self, message):
        ...
        for target in self._notifiers:
            await self.opp.services.async_call(DOMAIN_NOTIFY, target, msg_payload)
