...
def _write_refresh_token_to_file(opp, filename, refresh_token):
...
@asyncio.coroutine
def async_setup_scanner(opp, config, async_see, discovery_info=None):
    """Validate the configuration and return an Automatic scanner."""

    opp.http.register_view(AutomaticAuthCallbackView())
    ...
    refresh_token = yield from opp.async_add_job(
        _get_refresh_token_from_file, opp, filename
    )
    ...
    def initialize_data(session):
        ...
        opp.async_add_job(
            _write_refresh_token_to_file, opp, filename, session.refresh_token
        )
        data = AutomaticData(opp, client, session, config.get(CONF_DEVICES), async_see)

        # Load the initial vehicle data
        vehicles = yield from session.get_vehicles()
        for vehicle in vehicles:
            opp.async_create_task(data.load_vehicle(vehicle))

        # Create a task instead of adding a tracking job, since this task will
        # run until the websocket connection is closed.
        opp.loop.create_task(data.ws_connect())
    ...
    request_id = configurator.async_request_config(
        ...
        link_name="Click here to authorize Open Peer Power.",
    ...
    if DATA_CONFIGURING not in opp.data:
        opp.data[DATA_CONFIGURING] = {}

    opp.data[DATA_CONFIGURING][client.state] = initialize_callback
    return True


class AutomaticAuthCallbackView(OpenPeerPowerView):
    ...
    def get(self, request):  # pylint: disable=no-self-use
        ...
        if (
            DATA_CONFIGURING not in opp.data
            or params["state"] not in opp.data[DATA_CONFIGURING]
        ):
        ...
        code = params["code"]
        state = params["state"]
        initialize_callback = opp.data[DATA_CONFIGURING][state]
        opp.async_create_task(initialize_callback(code, state))
...
class AutomaticData:
    ...
    def __init__(self, opp, client, session, devices, async_see):
        ...
        self.client.on_app_event(
            lambda name, event: self.opp.async_create_task(
                self.handle_event(name, event)
            )
        )

        opp.bus.async_listen_once(EVENT_OPENPEERPOWER_STOP, self.ws_close())

    @asyncio.coroutine
    def handle_event(self, name, event):
        """Coroutine to update state for a real time event."""

        self.opp.bus.async_fire(EVENT_AUTOMATIC_UPDATE, event.data)
    ...
    def ws_connect(self, now=None):
        ...
        try:
            ...
            if self.ws_reconnect_handle is None:
                ...
                self.ws_reconnect_handle = async_track_time_interval(
                    self.opp, self.ws_connect, timedelta(minutes=5)
                )
            return
        ...
        if not self.ws_close_requested:
            self.opp.loop.create_task(self.ws_connect())
    ...
    def load_vehicle(self, vehicle):
        """Load the vehicle's initial state and update opp."""
