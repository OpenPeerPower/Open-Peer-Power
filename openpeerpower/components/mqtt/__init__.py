...
async def _async_setup_discovery(
    opp: OpenPeerPowerType, conf: ConfigType, opp_config: ConfigType, config_entry
) -> bool:
    ...
    success: bool = await discovery.async_start(
        opp, conf[CONF_DISCOVERY_PREFIX], opp_config, config_entry
    )
...
async def async_setup_entry(opp, entry):
    ...
    result: str = await opp.data[DATA_MQTT].async_connect()
...
class MQTT:
    ...
    async def _async_unsubscribe(self, topic: str) -> None:
        ...
        async with self._paho_lock:
            result: int = None
            result, _ = await self.opp.async_add_job(self._mqttc.unsubscribe, topic)
    ...
    async def _async_perform_subscription(self, topic: str, qos: int) -> None:
        ...
        async with self._paho_lock:
            result: int = None
            result, _ = await self.opp.async_add_job(self._mqttc.subscribe, topic, qos)
            _raise_on_error(result)
