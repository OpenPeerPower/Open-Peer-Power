...
async def async_setup_entry(opp, config_entry, async_add_entities):
    ...
    async def async_discover(discovery_payload):
        ...
        try:
            ...
            await _async_setup_entity(
                opp, config, async_add_entities, config_entry, discovery_hash
            )
        except Exception:
            if discovery_hash:
    ...
    async_dispatcher_connect(
        opp, MQTT_DISCOVERY_NEW.format(climate.DOMAIN, "mqtt"), async_discover
    )


async def _async_setup_entity(
    opp, config, async_add_entities, config_entry=None, discovery_hash=None
):
    """Set up the MQTT climate devices."""
    async_add_entities([MqttClimate(opp, config, config_entry, discovery_hash)])
