...
import openpeerpower.components.automation.mqtt as automation_mqtt
...
class Trigger:
    ...
    opp = attr.ib(type=OpenPeerPowerType)
...
async def async_setup_trigger(opp, config, config_entry, discovery_hash):
    ...
    if discovery_id not in opp.data[DEVICE_TRIGGERS]:
        ...
        await opp.data[DEVICE_TRIGGERS][discovery_id].update_trigger(config)
