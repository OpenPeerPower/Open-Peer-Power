...
class HKDevice:
    ...
    async def async_setup(self):
        ...
        if not cache:
            if await self.async_refresh_entity_map(self.config_num):
                self._polling_interval_remover = async_track_time_interval(
                    self.opp, self.async_update, DEFAULT_SCAN_INTERVAL
                )
                return True
            return False
        ...
        self.opp.async_create_task(self.async_process_entity_map())
    ...
    async def async_refresh_entity_map(self, config_num):
        ...
        try:
            async with self.pairing_lock:
                self.accessories = await self.opp.async_add_executor_job(
        ...
        self.opp.data[ENTITY_MAP].async_create_or_update_map(
    ...
    async def get_characteristics(self, *args, **kwargs):
        ...
        async with self.pairing_lock:
            chars = await self.opp.async_add_executor_job(
    ...
    async def put_characteristics(self, characteristics):
        ...
        async with self.pairing_lock:
            results = await self.opp.async_add_executor_job(
