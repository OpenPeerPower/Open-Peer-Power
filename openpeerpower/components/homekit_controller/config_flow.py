...
HOMEKIT_IGNORE = ["Open Peer Power Bridge"]
HOMEKIT_DIR = ".homekit"
...
def load_old_pairings(opp):
    """Load any old pairings from on-disk json fragments."""
    old_pairings = {}

    data_dir = os.path.join(opp.config.path(), HOMEKIT_DIR)
...
class HomekitControllerFlowHandler(config_entries.ConfigFlow):
    ...
    async def async_step_user(self, user_input=None):
        ...
        all_hosts = await self.opp.async_add_executor_job(self.controller.discover, 5)
    ...
    async def async_step_unignore(self, user_input):
        ...
        records = await self.opp.async_add_executor_job(self.controller.discover, 5)
    ...
    async def async_step_zeroconf(self, discovery_info):
        ...
        if paired and hkid in self.opp.data.get(KNOWN_DEVICES, {}):
            conn = self.opp.data[KNOWN_DEVICES][hkid]
            if conn.config_num != config_num:
                _LOGGER.debug(
                    "HomeKit info %s: c# incremented, refreshing entities", hkid
                )
                self.opp.async_create_task(conn.async_refresh_entity_map(config_num))
        ...
        if paired:
            old_pairings = await self.opp.async_add_executor_job(
                load_old_pairings, self.opp
            )
        ...
        existing = find_existing_host(self.opp, hkid)
        if existing:
            await self.opp.config_entries.async_remove(existing.entry_id)
    ...
    async def async_step_pair(self, pair_info=None):
        ...
        if pair_info:
            ...
            try:
                ...
                await self.opp.async_add_executor_job(self.finish_pairing, code)
        ...
        try:
            self.finish_pairing = await self.opp.async_add_executor_job(
    ...
    async def _entry_from_accessory(self, pairing):
        ...
        if not accessories:
            accessories = await self.opp.async_add_executor_job(
