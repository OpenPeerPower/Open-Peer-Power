...
from openpeerpower.const import (
    ...
    EVENT_AUTOMATION_TRIGGERED, EVENT_OPENPEERPOWER_START, SERVICE_RELOAD,
    SERVICE_TOGGLE, SERVICE_TURN_OFF, SERVICE_TURN_ON, STATE_ON)
from openpeerpower.core import Context, CoreState
from openpeerpower.exceptions import OpenPeerPowerError
from openpeerpower.helpers import condition, extract_domain_configs, script
...
class AutomationEntity(ToggleEntity, RestoreEntity):
    ...
    async def async_added_to_opp(self) -> None:
        ...
        if self.opp.state == CoreState.not_running:
            ...
            self.opp.bus.async_listen_once(
                EVENT_OPENPEERPOWER_START, async_enable_automation)

        # OpenPeerPower is running
        else:
            await self.async_enable()
    ...
    async def async_trigger(self, variables, skip_condition=False,
        ...
        self.opp.bus.async_fire(EVENT_AUTOMATION_TRIGGERED, {
...
async def _async_process_config(opp, config, component):
    ...
    for config_key in extract_domain_configs(config, DOMAIN):
        ...
        for list_no, config_block in enumerate(conf):
            ...
            action = _async_get_action(opp, config_block.get(CONF_ACTION, {}),
                                       name)

            if CONF_CONDITION in config_block:
                cond_func = _async_process_if(opp, config, config_block)
            ...
            async_attach_triggers = partial(
                _async_process_trigger, opp, config,
...
def _async_get_action(opp, config, name):
    """Return an action based on a configuration."""
    script_obj = script.Script(opp, config, name)
...
def _async_process_if(opp, config, p_config):
    """Process if checks."""
...
async def _async_process_trigger(opp, config, trigger_configs, name, action):
    """Set up the triggers.
    ...
    for conf in trigger_configs:
        ...
        remove = await platform.async_trigger(opp, conf, action, info)
