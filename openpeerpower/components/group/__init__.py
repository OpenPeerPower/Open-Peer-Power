...
@bind_opp
def expand_entity_ids(opp, entity_ids):
    ...
    for entity_id in entity_ids:
        ...
        try:
            ...
            if domain == DOMAIN:
                ...
                found_ids.extend(
                    ent_id for ent_id
                    in expand_entity_ids(opp, child_entities)
...
@bind_opp
def get_entity_ids(opp, entity_id, domain_filter=None):
    ...
    async def groups_service_handler(service):
        ...
        if service.service == SERVICE_SET and group is None:
            ...
            await Group.async_create_group(
                opp, service.data.get(ATTR_NAME, object_id),
                object_id=object_id,
...
async def _async_process_config(opp, config, component):
    ...
    for object_id, conf in config.get(DOMAIN, {}).items():
        ...
        await Group.async_create_group(
            opp, name, entity_ids, icon=icon, view=view,
...
class Group(Entity):
    ...
    def __init__(self, opp, name, order=None, visible=True, icon=None,
    ...
    @staticmethod
    def create_group(opp, name, entity_ids=None, user_defined=True,
                     visible=True, icon=None, view=False, control=None,
                     object_id=None, mode=None):
        """Initialize a group."""
        return run_coroutine_threadsafe(
            Group.async_create_group(
                opp, name, entity_ids, user_defined, visible, icon, view,
                control, object_id, mode),
            opp.loop).result()

    @staticmethod
    async def async_create_group(opp, name, entity_ids=None,
        ...
        if component is None:
            component = opp.data[DOMAIN] = \
                EntityComponent(_LOGGER, DOMAIN, opp)
    ...
    def update_tracked_entity_ids(self, entity_ids):
        ...
        run_coroutine_threadsafe(
            self.async_update_tracked_entity_ids(entity_ids), self.opp.loop
        ).result()
