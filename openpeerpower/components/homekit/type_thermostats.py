...
class Thermostat(HomeAccessory):
    ...
    def __init__(self, *args):
        ...
        temp_step = self.opp.states.get(self.entity_id).attributes.get(
        ...
        hc_valid_values = {k: v for v, k in self.hc_homekit_to_opp.items()}
    ...
    def get_temperature_range(self):
        ...
        max_temp = self.opp.states.get(self.entity_id).attributes.get(ATTR_MAX_TEMP)
        max_temp = (
        ...
        min_temp = self.opp.states.get(self.entity_id).attributes.get(ATTR_MIN_TEMP)
        min_temp = (
    ...
    def set_heat_cool(self, value):
        ...
        opp_value = self.hc_homekit_to_opp[value]
        params = {ATTR_ENTITY_ID: self.entity_id, ATTR_HVAC_MODE: opp_value}
        self.call_service(
            DOMAIN_CLIMATE, SERVICE_SET_HVAC_MODE_THERMOSTAT, params, opp_value
        )
    ...
    def update_state(self, new_state):
        ...
        if self._unit and self._unit in UNIT_OPP_TO_HOMEKIT:
            self.char_display_units.set_value(UNIT_OPP_TO_HOMEKIT[self._unit])

        # Update target operation mode
        hvac_mode = new_state.state
        if hvac_mode and hvac_mode in HC_OPP_TO_HOMEKIT:
            if not self._flag_heat_cool:
                self.char_target_heat_cool.set_value(HC_OPP_TO_HOMEKIT[hvac_mode])
        self._flag_heat_cool = False
