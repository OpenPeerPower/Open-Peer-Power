...
from openpeerpower.core import callback as op_callback, split_entity_id
...
def debounce(func):
    ...
    def call_later_listener(self, *args):
        ...
        if debounce_params:
            self.opp.async_add_executor_job(func, self, *debounce_params[1:])
...
class HomeAccessory(Accessory):
    ...
    async def run(self):
        ...
        """
        self.opp.add_job(self.run_handler)
    ...
    def update_state_callback(self, entity_id=None, old_state=None, new_state=None):
        ...
        if self._support_battery_level and not self.linked_battery_sensor:
            self.opp.async_add_executor_job(self.update_battery, new_state)
        self.opp.async_add_executor_job(self.update_state, new_state)
    ...
    async def async_call_service(self, domain, service, service_data, value=None):
        ...
        self.opp.bus.async_fire(EVENT_HOMEKIT_CHANGED, event_data)
        await self.opp.services.async_call(domain, service, service_data)
...
class HomeDriver(AccessoryDriver):
    ...
    def pair(self, client_uuid, client_public):
        ...
        if success:
            dismiss_setup_message(self.opp)
        return success

    def unpair(self, client_uuid):
        """Override super function to show setup message if unpaired."""
        super().unpair(client_uuid)
        show_setup_message(self.opp, self.state.pincode)
