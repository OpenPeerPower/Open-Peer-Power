...
async def async_setup(opp, config):
    ...
    await opp.async_add_executor_job(homekit.setup)
    ...
    if auto_start:
        opp.bus.async_listen_once(EVENT_OPENPEERPOWER_START, homekit.start)
        return True
...
class HomeKit:
    ...
    def start(self, *args):
        ...
        for state in self.opp.states.all():
            self.add_bridge_accessory(state)
        self.driver.add_accessory(self.bridge)

        if not self.driver.state.paired:
            show_setup_message(self.opp, self.driver.state.pincode)
        ...
        self.opp.add_job(self.driver.start)
    ...
    def stop(self, *args):
        ...
        self.opp.add_job(self.driver.stop)
