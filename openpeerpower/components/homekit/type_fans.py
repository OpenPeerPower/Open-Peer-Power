...
class Fan(HomeAccessory):
    ...
    def __init__(self, *args):
        ...
        chars = []
        features = self.opp.states.get(self.entity_id).attributes.get(
            ATTR_SUPPORTED_FEATURES
        )
        ...
        if features & SUPPORT_SET_SPEED:
            speed_list = self.opp.states.get(self.entity_id).attributes.get(
                ATTR_SPEED_LIST
            )
    ...
    def update_state(self, new_state):
        ...
        if self.char_speed is not None:
            ...
            if hk_speed_value is not None and self.char_speed.value != hk_speed_value:
                # If the openpeerpower component reports its speed as the first entry
