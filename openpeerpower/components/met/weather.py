...
from openpeerpower.components.weather import PLATFORM_SCHEMA, WeatherEntity
...
async def async_setup_platform(opp, config, async_add_entities,
    ...
    # Add defaults.
    config = {
        CONF_ELEVATION: opp.config.elevation,
        **config,
    }
...
class MetWeather(WeatherEntity):
    ...
    def _init_data(self):
        ...
        self._weather_data = metno.MetWeatherData(
            coordinates, async_get_clientsession(self.opp), URL)
    ...
    async def will_remove_from_opp(self):
        """Handle entity will be removed from opp."""
    ...
    async def _fetch_data(self, *_):
        ...
        if not await self._weather_data.fetching_data():
            ...
            self._unsub_fetch_data = async_call_later(
                self.opp, minutes*60, self._fetch_data)
            return
        ...
        self._unsub_fetch_data = async_call_later(
            self.opp, randrange(55, 65)*60, self._fetch_data)
        self._update()
    ...
    def name(self):
        ...
        if self.track_home:
            return self.opp.config.location_name
