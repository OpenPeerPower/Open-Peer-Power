...
async def async_request_stream(opp, entity_id, fmt):
    ...
    return request_stream(opp, source, fmt=fmt, keepalive=camera_prefs.preload_stream)
...
async def async_setup(opp, config):
    ...
    async def preload_stream(opp, _):
        for camera in component.entities:
            ...
            request_stream(opp, source, keepalive=True)
...
class Camera(Entity):
    ...
    async def async_camera_image(self):
        """Return bytes of camera image."""
        return await self.opp.async_add_job(self.camera_image)
...
async def ws_camera_stream(opp, connection, msg):
    ...
    try:
        ...
        fmt = msg["format"]
        url = request_stream(
            opp, source, fmt=fmt, keepalive=camera_prefs.preload_stream
        )
...
async def async_handle_play_stream_service(camera, service_call):
    ...
    url = request_stream(opp, source, fmt=fmt, keepalive=camera_prefs.preload_stream)
    data = {
