...
coverage-local: test-local
	...
	pep8 pyyasdi > pep8.report || echo "Not pep8 clean"

	# Compute pylint quality
	pylint -f parseable pyyasdi --msg-template "{path}:{line}: [{msg_id}({symbol}), {obj}] {msg}" > pylint.report || echo "Not pylint clean"
