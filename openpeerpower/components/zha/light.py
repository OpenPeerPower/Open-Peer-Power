...
async def async_setup_entry(opp, config_entry, async_add_entities):
    ...
    unsub = async_dispatcher_connect(
        opp, ZHA_DISCOVERY_NEW.format(light.DOMAIN), async_discover
    )
    opp.data[DATA_ZHA][DATA_ZHA_DISPATCHERS].append(unsub)

    lights = opp.data.get(DATA_ZHA, {}).get(light.DOMAIN)
    if lights is not None:
        await _async_setup_entities(
            opp, config_entry, async_add_entities, lights.values()
        )
