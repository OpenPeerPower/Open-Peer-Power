...
from openpeerpower import const as op_const
...
def async_process_endpoint(
    ...
    if op_const.CONF_TYPE in node_config:
        component = node_config[op_const.CONF_TYPE]
    ...
    discovery_infos.extend(
        _async_handle_single_cluster_matches(
            opp, endpoint, zha_device, profile_clusters, device_key, is_new_join
        )
    )
...
@callback
def async_dispatch_discovery_info(opp, is_new_join, discovery_info):
    ...
    if is_new_join:
        async_dispatcher_send(opp, ZHA_DISCOVERY_NEW.format(component), discovery_info)
    else:
        opp.data[DATA_ZHA][component][discovery_info["unique_id"]] = discovery_info


@callback
def _async_handle_profile_match(
    opp, endpoint, profile_clusters, zha_device, component, device_key, is_new_join
):
...
def _async_handle_single_cluster_match(
    opp, zha_device, cluster, device_key, device_classes, is_new_join
):
