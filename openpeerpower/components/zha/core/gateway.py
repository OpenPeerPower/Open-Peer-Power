...
from openpeerpower.components.system_log import LogEntry, _figure_out_source
...
class ZHAGateway:
    ...
    def __init__(self, opp, config, config_entry):
        ...
        self.op_device_registry = None
        self.op_entity_registry = None
        self.application_controller = None
        self.radio_description = None
        opp.data[DATA_ZHA][DATA_ZHA_GATEWAY] = self
        self._log_levels = {
    ...
    async def async_initialize(self):
        ...
        self.op_device_registry = await get_dev_reg(self._opp)
        self.op_entity_registry = await get_ent_reg(self._opp)
        ...
        if CONF_DATABASE in self._config:
            ...
            database = os.path.join(self._opp.config.config_dir, DEFAULT_DATABASE_NAME)
    ...
    async def _async_remove_device(self, device, entity_refs):
        ...
        reg_device = self.op_device_registry.async_get(device.device_id)
        if reg_device is not None:
            self.op_device_registry.async_remove_device(reg_device.id)
    ...
    def _async_get_or_create_device(self, zigpy_device):
        ...
        if zha_device is None:
            zha_device = ZHADevice(self._opp, zigpy_device, self)
            self._devices[zigpy_device.ieee] = zha_device
            device_registry_device = self.op_device_registry.async_get_or_create(
    ...
    async def _async_device_joined(self, device, zha_device):
        ...
        for discovery_info in discovery_infos:
            async_dispatch_discovery_info(self._opp, True, discovery_info)
    ...
    async def async_device_restored(self, device):
        ...
        for discovery_info in discovery_infos:
            async_dispatch_discovery_info(self._opp, False, discovery_info)
