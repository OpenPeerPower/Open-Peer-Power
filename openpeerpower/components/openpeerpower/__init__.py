...
async def async_setup(opp: op.OpenPeerPower, config: dict) -> Awaitable[bool]:
    ...
    async def async_handle_turn_service(service):
        ...
        for domain, ent_ids in by_domain:
            ...
            # But services can be registered on other OP instances that are
            # listening to the bus too. So as an in between solution, we'll
            # block only if the service is defined in the current OP instance.
    ...
    opp.services.async_register(op.DOMAIN, SERVICE_TURN_OFF, async_handle_turn_service)
    opp.services.async_register(op.DOMAIN, SERVICE_TURN_ON, async_handle_turn_service)
    opp.services.async_register(op.DOMAIN, SERVICE_TOGGLE, async_handle_turn_service)
    opp.helpers.intent.async_register(
        intent.ServiceIntentHandler(
            intent.INTENT_TURN_ON, op.DOMAIN, SERVICE_TURN_ON, "Turned {} on"
        )
    )
    opp.helpers.intent.async_register(
        intent.ServiceIntentHandler(
            intent.INTENT_TURN_OFF, op.DOMAIN, SERVICE_TURN_OFF, "Turned {} off"
        )
    )
    opp.helpers.intent.async_register(
        intent.ServiceIntentHandler(
            intent.INTENT_TOGGLE, op.DOMAIN, SERVICE_TOGGLE, "Toggled {}"
        )
    )
