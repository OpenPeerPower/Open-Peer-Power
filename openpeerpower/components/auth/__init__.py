...
Open Peer Power. User need to record the token in secure place.
...
from openpeerpower.auth.models import (
...
async def async_setup(opp, config):
    ...
    opp.data[DOMAIN] = store_result

    opp.http.register_view(TokenView(retrieve_result))
    opp.http.register_view(LinkUserView(retrieve_result))
...
class TokenView(OpenPeerPowerView):
    ...
    async def post(self, request):
        ...
        if grant_type == "authorization_code":
            return await self._async_handle_auth_code(
                opp, data, str(request[KEY_REAL_IP])
            )
    ...
    async def _async_handle_auth_code(self, opp, data, remote_addr):
        ...
        # refresh user
        user = await opp.auth.async_get_user(user.id)
        ...
        access_token = opp.auth.async_create_access_token(refresh_token, remote_addr)
