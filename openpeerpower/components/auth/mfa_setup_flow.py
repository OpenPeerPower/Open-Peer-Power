...
async def async_setup(opp):
    ...
    async def _async_create_setup_flow(handler, context, data):
        ...
        mfa_module = opp.auth.get_auth_mfa_module(handler)
    ...
    opp.data[DATA_SETUP_FLOW_MGR] = data_entry_flow.FlowManager(
        opp, _async_create_setup_flow, _async_finish_setup_flow)
