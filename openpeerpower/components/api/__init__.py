...
from openpeerpower.const import (
    EVENT_OPENPEERPOWER_STOP, EVENT_TIME_CHANGED, HTTP_BAD_REQUEST,
...
class APIStatesView(OpenPeerPowerView):
    ...
    def get(self, request):
        ...
        states = [
            state for state in request.app['opp'].states.async_all()
...
class APIEventView(OpenPeerPowerView):
    ...
    async def post(self, request, event_type):
        ...
        request.app['opp'].bus.async_fire(
            event_type, event_data, op.EventOrigin.remote,
            self.context(request))
