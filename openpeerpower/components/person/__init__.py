...
from openpeerpower.auth import EVENT_USER_REMOVED
...
async def async_create_person(opp, name, *, user_id=None, device_trackers=None):
    """Create a new person."""
    await opp.data[DOMAIN].async_create_person(
...
class PersonManager:
    ...
    def __init__(
        self, opp: OpenPeerPowerType, component: EntityComponent, config_persons
    ):
        """Initialize person storage."""
        self.opp = opp
        self.component = component
        self.store = Store(opp, STORAGE_VERSION, STORAGE_KEY)
    ...
    async def async_initialize(self):
        ...
        for person_conf in self.config_data.values():
            ...
            if user_id is not None:
                if await self.opp.auth.async_get_user(user_id) is None:
        ...
        self.opp.bus.async_listen(EVENT_USER_REMOVED, self._user_removed)
...
async def async_setup(opp: OpenPeerPowerType, config: ConfigType):
    ...
    manager = opp.data[DOMAIN] = PersonManager(opp, component, conf_persons)
    await manager.async_initialize()

    websocket_api.async_register_command(opp, ws_list_person)
    websocket_api.async_register_command(opp, ws_create_person)
    websocket_api.async_register_command(opp, ws_update_person)
    websocket_api.async_register_command(opp, ws_delete_person)
...
def ws_list_person(
    ...
    """List persons."""
    manager: PersonManager = opp.data[DOMAIN]
