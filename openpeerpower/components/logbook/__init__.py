...
from openpeerpower.components.recorder.util import (
    QUERY_RETRY_WAIT,
    RETRIES,
    session_scope,
)
...
from openpeerpower.core import DOMAIN as HA_DOMAIN, State, callback, split_entity_id
...
@bind_opp
def log_entry(opp, name, message, domain=None, entity_id=None):
    """Add an entry to the logbook."""
    opp.add_job(async_log_entry, opp, name, message, domain, entity_id)


@bind_opp
def async_log_entry(opp, name, message, domain=None, entity_id=None):
    """Add an entry to the logbook."""
    ...
    opp.bus.async_fire(EVENT_LOGBOOK_ENTRY, data)
...
async def async_setup(opp, config):
    ...
    opp.http.register_view(LogbookView(config.get(DOMAIN, {})))
