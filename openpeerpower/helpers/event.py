...
from openpeerpower.core import CALLBACK_TYPE, Event, OpenPeerPower, State, callback
...
def async_track_template(
    ...
    return async_track_state_change(
        opp, template.extract_entities(variables), template_condition_listener
    )
...
def async_call_later(
    opp: OpenPeerPower, delay: float, action: Callable[..., None]
) -> CALLBACK_TYPE:
...
def async_track_time_interval(
    opp: OpenPeerPower, action: Callable[..., None], interval: timedelta
) -> CALLBACK_TYPE:
    ...
    def interval_listener(now: datetime) -> None:
        ...
        nonlocal remove
        remove = async_track_point_in_utc_time(opp, interval_listener, next_interval())
        opp.async_run_job(action, now)
...
class SunListener:
    ...
    opp = attr.ib(type=OpenPeerPower)
    ...
    def _handle_sun_event(self, _now: Any) -> None:
        ...
        self._unsub_sun = None
        self._listen_next_sun_event()
        self.opp.async_run_job(self.action)
...
def async_track_sunrise(
    ...
    listener = SunListener(opp, action, SUN_EVENT_SUNRISE, offset)
...
def async_track_sunset(
    ...
    listener = SunListener(opp, action, SUN_EVENT_SUNSET, offset)
    listener.async_attach()
    return listener.async_detach
