...
from openpeerpower.core import (
    OpenPeerPower, callback, State, CoreState, valid_entity_id)
...
from openpeerpower.helpers.storage import Store  # noqa  pylint_disable=unused-import
...
class RestoreStateData():
    ...
    async def async_get_instance(
            cls, opp: OpenPeerPower) -> 'RestoreStateData':
        """Get the singleton instance of this data helper."""
        task = opp.data.get(DATA_RESTORE_STATE_TASK)
        ...
        if task is None:
            ...
            task = opp.data[DATA_RESTORE_STATE_TASK] = opp.async_create_task(
                load_instance(opp))
    ...
    def __init__(self, opp: OpenPeerPower) -> None:
        ...
        self.opp = opp  # type: OpenPeerPower
        self.store = Store(
            opp, STORAGE_VERSION, STORAGE_KEY,
    ...
    def async_restore_entity_removed(self, entity_id: str) -> None:
        ...
        self.last_states[entity_id] = StoredState(
            self.opp.states.get(entity_id), dt_util.utcnow())
...
class RestoreEntity(Entity):
    ...
    async def async_will_remove_from_opp(self) -> None:
        ...
        _, data = await asyncio.gather(
            super().async_will_remove_from_opp(),
            RestoreStateData.async_get_instance(self.opp),
        )
