...
class AbstractOAuth2Implementation(ABC):
    ...
    async def async_generate_authorize_url(self, flow_id: str) -> str:
        ...
        user to the vendor website where they can authorize Open Peer Power.
...
class LocalOAuth2Implementation(AbstractOAuth2Implementation):
    ...
    def redirect_uri(self) -> str:
        ...
        return f"{self.opp.config.api.base_url}{AUTH_CALLBACK_PATH}"  # type: ignore
            ...
            URL(self.authorize_url).with_query(
                {
                    ...
                    "state": _encode_jwt(self.opp, {"flow_id": flow_id}),
                }
            )
        )
...
class AbstractOAuth2FlowHandler(config_entries.ConfigFlow, metaclass=ABCMeta):
    ...
    async def async_step_pick_implementation(self, user_input: dict = None) -> dict:
        """Handle a flow start."""
        assert self.opp
        implementations = await async_get_implementations(self.opp, self.DOMAIN)
...
def async_add_implementation_provider(
    ...
    """
    opp.data.setdefault(DATA_PROVIDERS, {})[
        provider_domain
    ] = async_provide_implementation


class OAuth2AuthorizeCallbackView(OpenPeerPowerView):
...
class OAuth2Session:
    ...
    async def async_request(
        ...
        return await async_oauth2_request(
            self.opp, self.config_entry.data["token"], method, url, **kwargs
        )
