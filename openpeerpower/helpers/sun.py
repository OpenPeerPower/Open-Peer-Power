...
def get_astral_event_next(
        opp: OpenPeerPowerType, event: str,
...
@callback
@bind_opp
def is_up(opp: OpenPeerPowerType,
          utc_point_in_time: Optional[datetime.datetime] = None) -> bool:
    """Calculate if the sun is currently up."""
    if utc_point_in_time is None:
        utc_point_in_time = dt_util.utcnow()

    next_sunrise = get_astral_event_next(opp, SUN_EVENT_SUNRISE,
                                         utc_point_in_time)
    next_sunset = get_astral_event_next(opp, SUN_EVENT_SUNSET,
                                        utc_point_in_time)
