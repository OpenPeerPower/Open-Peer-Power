...
@bind_opp
async def async_migrator(opp, old_path, store, *,
...
class Store:
    ...
    def __init__(self, opp, version: int, key: str, private: bool = False, *,
        ...
        self._write_lock = asyncio.Lock(loop=opp.loop)
    ...
    async def async_load(self) -> Optional[Union[Dict, List]]:
        ...
        if self._load_task is None:
            self._load_task = self.opp.async_add_job(self._async_load())
    ...
    def _async_ensure_stop_listener(self):
        ...
        if self._unsub_stop_listener is None:
            self._unsub_stop_listener = self.opp.bus.async_listen_once(
                EVENT_OPENPEERPOWER_STOP, self._async_callback_stop_write)
    ...
    async def _async_callback_stop_write(self, _event):
        """Handle a write because Open Peer Power is stopping."""
        self._unsub_stop_listener = None
        self._async_cleanup_delay_listener()
        await self._async_handle_write_data()
