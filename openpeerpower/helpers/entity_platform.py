...
from openpeerpower.core import callback, valid_entity_id, split_entity_id
...
class EntityPlatform:
    ...
    def __init__(self, *, opp, logger, domain, platform_name, platform,
        ...
        self._process_updates = asyncio.Lock(loop=opp.loop)
    ...
    async def async_setup(self, platform_config, discovery_info=None):
        ...
        def async_create_setup_task():
            ...
            return opp.loop.run_in_executor(
                None, platform.setup_platform, opp, platform_config,
    ...
    async def _async_setup_platform(self, async_create_setup_task, tries=0):
        ...
        try:
            ...
            await asyncio.wait_for(
                asyncio.shield(task, loop=opp.loop),
                SLOW_SETUP_MAX_WAIT, loop=opp.loop)
            ...
            self._async_cancel_retry_setup = \
                async_call_later(opp, wait_time, setup_again)
            return False
    ...
    def _async_schedule_add_entities(self, new_entities,
        ...
        self._tasks.append(self.opp.async_add_job(
    ...
    async def _async_add_entity(self, entity, update_before_add,
        ...
        if (entity.entity_id in self.entities or
                entity.entity_id in self.opp.states.async_entity_ids(
                    self.domain)):
