...
class DiscoveryFlowHandler(config_entries.ConfigFlow):
    ...
    async def async_step_confirm(self, user_input=None):
        ...
        if self.context and self.context.get('source') != \
            ...
            if not has_devices:
                has_devices = await self.opp.async_add_job(
...
class WebhookFlowHandler(config_entries.ConfigFlow):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if self.opp.components.cloud.async_active_subscription():
            webhook_url = \
                await self.opp.components.cloud.async_create_cloudhook(
                    webhook_id
                )
            cloudhook = True
        else:
            webhook_url = \
                self.opp.components.webhook.async_generate_url(webhook_id)
            cloudhook = False
...
async def webhook_async_remove_entry(opp, entry) -> None:
    """Remove a webhook config entry."""
    if (not entry.data.get('cloudhook') or
            'cloud' not in opp.config.components):
        return
