...
class RegistryEntry:
    ...
    disabled_by = attr.ib(
        ...
        validator=attr.validators.in_((DISABLED_OPP, DISABLED_USER, None)))
...
class EntityRegistry:
    ...
    def async_get_or_create(self, domain, platform, unique_id, *,
        ...
        self.opp.bus.async_fire(EVENT_ENTITY_REGISTRY_UPDATED, {
            'action': 'create',
            'entity_id': entity_id
        })
    ...
    async def async_load(self):
        ...
        data = await self.opp.helpers.storage.async_migrator(
            self.opp.config.path(PATH_REGISTRY), self._store,
...
async def async_get_registry(opp: OpenPeerPowerType) -> EntityRegistry:
    ...
    if not reg_or_evt:
        evt = opp.data[DATA_REGISTRY] = Event()
