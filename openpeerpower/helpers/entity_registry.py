...
from openpeerpower.util import ensure_unique_string, slugify
from openpeerpower.util.yaml import load_yaml

from .typing import OpenPeerPowerType

if TYPE_CHECKING:
    from openpeerpower.config_entries import ConfigEntry  # noqa: F401
...
class EntityRegistry:
    ...
    def async_generate_entity_id(
        ...
        return ensure_unique_string(
            ...
            chain(
                self.entities.keys(),
                self.opp.states.async_entity_ids(domain),
...
async def async_get_registry(opp: OpenPeerPowerType) -> EntityRegistry:
    ...
    reg_or_evt = opp.data.get(DATA_REGISTRY)

    if not reg_or_evt:
        evt = opp.data[DATA_REGISTRY] = asyncio.Event()

        reg = EntityRegistry(opp)
        await reg.async_load()

        opp.data[DATA_REGISTRY] = reg
        evt.set()
        return reg
    ...
    if isinstance(reg_or_evt, asyncio.Event):
        evt = reg_or_evt
        await evt.wait()
        return cast(EntityRegistry, opp.data.get(DATA_REGISTRY))
...
def async_setup_entity_restore(
    ...
    def cleanup_restored_states(event: Event) -> None:
        ...
        opp.states.async_remove(event.data["entity_id"])

    opp.bus.async_listen(EVENT_ENTITY_REGISTRY_UPDATED, cleanup_restored_states)
