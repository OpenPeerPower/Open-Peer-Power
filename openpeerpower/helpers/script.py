...
from openpeerpower.core import OpenPeerPower, Context, callback
from openpeerpower.const import CONF_CONDITION, CONF_TIMEOUT
...
def call_from_config(opp: OpenPeerPower, config: ConfigType,
...
class Script():
    ...
    def __init__(self, opp: OpenPeerPower, sequence, name: str = None,
    ...
    def run(self, variables=None, context=None):
        ...
        run_coroutine_threadsafe(
            self.async_run(variables, context), self.opp.loop).result()
    ...
    async def _async_wait_template(self, action, variables, context):
        ...
        if condition.async_template(
                self.opp, wait_template, variables):
            return
        ...
        self._async_listener.append(async_track_template(
            self.opp, wait_template, async_script_wait, variables))
    ...
    async def _async_fire_event(self, action, variables, context):
        ...
        self.opp.bus.async_fire(action[CONF_EVENT],
