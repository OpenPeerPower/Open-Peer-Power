...
import openpeerpower.components.device_automation as device_automation
...
def call_from_config(
    ...
    Script(opp, cv.SCRIPT_SCHEMA(config)).run(variables, context)
...
class Script:
        ...
        for cur, action in islice(enumerate(self.sequence), self._cur, None):
            try:
                ...
                if self._change_listener:
                    self.opp.async_add_job(self._change_listener)
                return
            except _StopScript:
                break
            except Exception:
    ...
    def stop(self) -> None:
        """Stop running script."""
        run_callback_threadsafe(self.opp.loop, self.async_stop).result()
    ...
    async def _async_delay(self, action, variables, context):
        ...
        unsub = async_track_point_in_utc_time(
            self.opp, async_script_delay, date_util.utcnow() + delay
        )
    ...
    async def _async_wait_template(self, action, variables, context):
        ...
        if condition.async_template(self.opp, wait_template, variables):
            return
        ...
        self._async_listener.append(
            async_track_template(self.opp, wait_template, async_script_wait, variables)
        )
    ...
    async def _async_device_automation(self, action, variables, context):
        ...
        self._log("Executing step %s" % self.last_action)
        platform = await device_automation.async_get_device_automation_platform(
            self.opp, action[CONF_DOMAIN], "action"
        )
        await platform.async_call_action_from_config(
            self.opp, action, variables, context
        )
    ...
    async def _async_fire_event(self, action, variables, context):
        ...
        self.opp.bus.async_fire(action[CONF_EVENT], event_data, context=context)
    ...
    async def _async_check_condition(self, action, variables, context):
        ...
        if not config:
            config = await condition.async_from_config(self.opp, action, False)
            self._config_cache[config_cache_key] = config

        self.last_action = action.get(CONF_ALIAS, action[CONF_CONDITION])
        check = config(self.opp, variables)
    ...
    def _async_set_timeout(self, action, variables, context, continue_on_timeout):
        ...
        unsub = async_track_point_in_utc_time(
            self.opp, async_script_timeout, date_util.utcnow() + timeout
        )
        self._async_listener.append(unsub)
