...
from openpeerpower.components.sun import (
    STATE_ABOVE_HORIZON, STATE_BELOW_HORIZON)
from openpeerpower.components.mysensors.switch import (
...
class AsyncTrackStates:
    ...
    def __exit__(self, exc_type: Optional[Type[BaseException]],
        ...
        self.states.extend(get_changed_since(self.opp.states.async_all(),
                                             self.now))
...
@bind_opp
def reproduce_state(opp: OpenPeerPowerType,
                    states: Union[State, Iterable[State]],
                    blocking: bool = False) -> None:
    """Reproduce given state."""
    return run_coroutine_threadsafe(  # type: ignore
        async_reproduce_state(opp, states, blocking), opp.loop).result()
...
async def async_reproduce_state_legacy(
    ...
    for state in states:
        ...
        if opp.states.get(state.entity_id) is None:
            _LOGGER.warning("reproduce_state: Unable to find entity %s",
                            state.entity_id)
            continue

        domain_services = opp.services.async_services().get(service_domain)
    ...
    for (service, service_data), entity_ids in to_call.items():
        ...
        domain_tasks.append(
            opp.services.async_call(service_domain, service, data, blocking,
                                     context)
        )

    if domain_tasks:
        await asyncio.wait(domain_tasks, loop=opp.loop)
