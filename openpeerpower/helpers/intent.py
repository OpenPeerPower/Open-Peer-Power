...
from openpeerpower.const import ATTR_SUPPORTED_FEATURES
from openpeerpower.core import callback, State, T
...
@bind_opp
async def async_handle(opp: OpenPeerPowerType, platform: str,
                       intent_type: str, slots: Optional[_SlotsType] = None,
                       text_input: Optional[str] = None) -> 'IntentResponse':
    """Handle an intent."""
    handler = \
        opp.data.get(DATA_KEY, {}).get(intent_type)  # type: IntentHandler
...
@callback
@bind_opp
def async_match_state(opp: OpenPeerPowerType, name: str,
...
class ServiceIntentHandler(IntentHandler):
    ...
    async def async_handle(self, intent_obj: 'Intent') -> 'IntentResponse':
        ...
        await opp.services.async_call(self.domain, self.service, {
...
class Intent:
    ...
    __slots__ = ['opp', 'platform', 'intent_type', 'slots', 'text_input']

    def __init__(self, opp: OpenPeerPowerType, platform: str,
