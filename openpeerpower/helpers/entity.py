...
def generate_entity_id(entity_id_format: str, name: Optional[str],
                       ...
                       opp: Optional[OpenPeerPower] = None) -> str:
    """Generate a unique entity ID based on given entity IDs or used IDs."""
    if current_ids is None:
        if opp is None:
            raise ValueError("Missing required parameter currentids or opp")
        return run_callback_threadsafe(
            opp.loop, async_generate_entity_id, entity_id_format, name,
            current_ids, opp
        ).result()
...
class Entity:
    ...
    opp = None  # type: Optional[OpenPeerPower]
    ...
    def async_registry_updated(self, old, new):
        ...
        self.opp.async_create_task(readd())
