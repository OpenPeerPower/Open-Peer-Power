...
from openpeerpower.exceptions import NoEntitySpecifiedError
...
def async_generate_entity_id(
    ...
    if current_ids is None:
        ...
        current_ids = opp.states.async_entity_ids()
...
class Entity(ABC):
    ...
    def async_write_op_state(self) -> None:
        ...
        self._async_write_op_state()  # type: ignore
        ...
        if end - start > 0.4 and not self._slow_reported:
            ...
            if "custom_components" in type(self).__module__:
                ...
                extra = (
                    ...
                    "https://github.com/open-peer-power/open-peer-power/issues?q=is%3Aopen+is%3Aissue"
                )
                if self.platform:
                    extra += (
    ...
    def schedule_update_op_state(self, force_refresh=False):
        """Schedule an update op state change task.

        Scheduling the update avoids executor deadlocks.

        Entity state and attributes are read when the update op state change
        task is executed.
        If state is changed more than once before the op state change task has
    ...
    async def async_device_update(self, warning=True):
        ...
        if warning:
            update_warn = self.opp.loop.call_later(
                SLOW_UPDATE_WARNING,
                _LOGGER.warning,
    ...
    async def async_remove(self) -> None:
        ...
        self.opp.states.async_remove(self.entity_id)
...
class ToggleEntity(Entity):
    ...
    async def async_turn_on(self, **kwargs):
        """Turn the entity on."""
        await self.opp.async_add_job(ft.partial(self.turn_on, **kwargs))
    ...
    async def async_turn_off(self, **kwargs):
        """Turn the entity off."""
        await self.opp.async_add_job(ft.partial(self.turn_off, **kwargs))
