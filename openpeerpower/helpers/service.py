...
from openpeerpower.exceptions import (
    OpenPeerPowerError, TemplateError, Unauthorized, UnknownUser)
...
@bind_opp
def call_from_config(opp, config, blocking=False, variables=None,
                     validate_config=True):
    """Call a service based on a config hash."""
    run_coroutine_threadsafe(
        async_call_from_config(opp, config, blocking, variables,
                               validate_config), opp.loop).result()


@bind_opp
async def async_call_from_config(opp, config, blocking=False, variables=None,
...
async def async_extract_entity_ids(opp, service_call, expand_group=True):
    ...
    if entity_ids:
        ...
        if expand_group:
            entity_ids = \
                opp.components.group.expand_entity_ids(entity_ids)
    ...
    if area_ids:
        ...
        devices = [
            ...
            for device in
            opp.helpers.device_registry.async_entries_for_area(
                dev_reg, area_id)
        ]
        extracted.update(
            entry.entity_id
...
async def _load_services_file(opp: OpenPeerPowerType, domain: str):
...
@bind_opp
async def entity_service_call(opp, platforms, func, call, service_name='',
...
def async_register_admin_service(
        opp: typing.OpenPeerPowerType, domain: str,
    ...
    async def admin_handler(call):
        ...
        await opp.async_add_job(service_func, call)
