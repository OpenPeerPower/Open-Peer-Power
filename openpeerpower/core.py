...
from openpeerpower.const import (
    ...
    EVENT_OPENPEERPOWER_START, EVENT_OPENPEERPOWER_STOP,
    EVENT_OPENPEERPOWER_CLOSE, EVENT_SERVICE_REMOVED,
...
from openpeerpower.exceptions import (
    OpenPeerPowerError, InvalidEntityFormatError, InvalidStateError,
...
from openpeerpower.util import location, slugify
from openpeerpower.util.unit_system import UnitSystem, METRIC_SYSTEM  # NOQA
...
class OpenPeerPower:
    ...
    async def async_run(self, *, attach_signals: bool = True) -> int:
        ...
        if attach_signals:
            from openpeerpower.helpers.signal \
...
class EventBus:
    ...
    def listeners(self) -> Dict[str, int]:
        ...
        return run_callback_threadsafe(  # type: ignore
            self._opp.loop, self.async_listeners
        ).result()
    ...
    def async_fire(self, event_type: str, event_data: Optional[Dict] = None,
        ...
        if (match_all_listeners is not None and
                event_type != EVENT_OPENPEERPOWER_CLOSE):
        ...
        for func in listeners:
            self._opp.async_add_job(func, event)
    ...
    def listen(
        ...
        async_remove_listener = run_callback_threadsafe(
            self._opp.loop, self.async_listen, event_type, listener).result()

        def remove_listener() -> None:
            """Remove the listener."""
            run_callback_threadsafe(
                self._opp.loop, async_remove_listener).result()
...
class ServiceRegistry:
    ...
    def services(self) -> Dict[str, Dict[str, Service]]:
        ...
        return run_callback_threadsafe(  # type: ignore
            self._opp.loop, self.async_services,
        ).result()
    ...
    def remove(self, domain: str, service: str) -> None:
        ...
        run_callback_threadsafe(
            self._opp.loop, self.async_remove, domain, service).result()
    ...
    async def async_call(self, domain: str, service: str,
        ...
        self._opp.bus.async_fire(EVENT_CALL_SERVICE, {
...
def _async_create_timer(opp: OpenPeerPower) -> None:
    ...
    def fire_time_event(target: float) -> None:
        ...
        if late > 1:
            opp.bus.async_fire(EVENT_TIMER_OUT_OF_SYNC,
                                {ATTR_SECONDS: late})
