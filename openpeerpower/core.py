...
from openpeerpower.util.async_ import fire_coroutine_threadsafe, run_callback_threadsafe
...
class ServiceRegistry:
    ...
    async def async_call(
        ...
        if not blocking:
            self._opp.async_create_task(self._safe_execute(handler, service_call))
            return None
...
def _async_create_timer(opp: OpenPeerPower) -> None:
    ...
    def fire_time_event(target: float) -> None:
        ...
        opp.bus.async_fire(EVENT_TIME_CHANGED, {ATTR_NOW: now})

        # If we are more than a second late, a tick was missed
        late = monotonic() - target
        if late > 1:
            opp.bus.async_fire(EVENT_TIMER_OUT_OF_SYNC, {ATTR_SECONDS: late})
