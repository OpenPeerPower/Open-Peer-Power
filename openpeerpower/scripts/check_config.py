...
from openpeerpower import bootstrap, core, loader, requirements
...
MOCKS = {
    'load': ("openpeerpower.util.yaml.loader.load_yaml",
             yaml_loader.load_yaml),
    'load*': ("openpeerpower.config.load_yaml", yaml_loader.load_yaml),
    'secrets': ("openpeerpower.util.yaml.loader.secret_yaml",
                yaml_loader.secret_yaml),
}
SILENCE = (
    'openpeerpower.scripts.check_config.yaml_loader.clear_secret_cache',
)
...
async def check_ha_config_file(opp):
    ...
    def _pack_error(package, component, config, message):
        ...
        domain = 'openpeerpower.packages.{}.{}'.format(package, component)
    ...
    try:
        config_path = await opp.async_add_executor_job(
    ...
    for domain in components:
        ...
        for p_name, p_config in config_per_platform(config, domain):
            ...
            try:
                p_integration = await loader.async_get_integration(opp,
                                                                   p_name)
            except loader.IntegrationNotFound:
                result.add_error(
